...
document {Key => {example,(example,ZZ),(example,Nothing,ZZ),(example,ZZ,Nothing),[example,Verbose]}, 
Headline => "examples of Trisecant Flops (old version)", 
Usage => "example i", 
...
PARA{"In the following, we consider the third example and we call some functions to analyze the data."},
EXAMPLE {"mu = example 3;","describe inverse mu","S = specialBaseLocus mu;",
         "? S", "parametrizeSpecialBaseLocus mu;", "U = specialBaseLocus inverse mu;","U == nonMinimalAssociatedK3surface 3","? U","mu' = extend mu;",
...
Inputs => {"i" => ZZ => {"an integer between 0 and 17"}},
Outputs => {{"the i-th example of birational map ",TEX///$X\dashrightarrow W$///," in accordance to the Table 1 in the paper ",HREF{"https://arxiv.org/abs/1909.01263","Trisecant Flops, their associated K3 surfaces and the rationality of some Fano fourfolds"},"."}}, 
PARA{"In the following, we consider the 7-th example and we call some functions to analyze the data."},
EXAMPLE {"psi = exampleMap 7;","X = source psi;","describe X","Y = target psi;","describe Y",
         "S = surface X;","T = surface Y;","psi;","eta = inverse psi;",
         "Psi = extend psi;","F = Psi^* point Y;","(describe F,describe (F*S))",
         "Eta = extend eta;","G = Eta^* point X;","(describe G,describe (G*T))"},
SeeAlso => {example}}
undocumented {specialBaseLocus,parametrizeSpecialBaseLocus,
              nonMinimalAssociatedK3surface,(nonMinimalAssociatedK3surface,ZZ),
              specialRationalMap,
...
-- document {Key => {parametrizeSpecialBaseLocus}, 
...
-- document {Key => {randomQuinticDelPezzoSurfaceIntersectingTheSpecialBaseLocusOfExample7AlongADegree10EGenus6Curve}, 
